...
def iniciar_jogo():
    ...
    root.right.left.left = Node("Você pode acusar a esposa com base na pegada", final=True)
    ...
    root.right.right.left = Node("Você acusa o sócio com base no álibi, mas não há provas suficientes.", final=True)
    root.right.right.right = arvore.add("Ver a reação do suspeito",
                                        "Você decide acusar alguém com base no objeto.",
                                        "Você continua procurando mais informações.")

    root.right.left.right.left = Node("Você acusa a esposa com base no depoimento, mas não há provas suficientes.", final=True)
    ...
    root.right.right.right.left = Node("Tentar etc", final=True)
